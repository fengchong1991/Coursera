    ...
    public static void runParallelFuzzyBarrier(final int iterations,
    	...
    	for(int i = 0; i < tasks; i++) {
    		...
    		threads[i] = new Thread(() -> {
				double[] myValLocal = myVal;
				double[] myNewLocal = myNew;
				    				
    			for(int iter = 0; iter < iterations; iter++) {

     				int left = ii*(n/tasks) + 1;
    				int right = (ii+1)*(n/tasks);
    				
    				myNewLocal[left] = (myValLocal[left-1] + myValLocal[left+1])/2.0;
    				
    				myNewLocal[right] = (myValLocal[right-1] + myValLocal[right+1])/2.0;
    				
    				int currentPhase = ph.arrive();
    				
    				for(int j = left+1; j <= right-1; j++) {
    					myNewLocal[j] = (myValLocal[j-1] + myValLocal[j+1])/2.0;
    				}
    				
    				ph.awaitAdvance(currentPhase);
    				
    				double[] temp = myNewLocal;
    				myNewLocal = myValLocal;
    				myValLocal = temp;
    			}
    		});
...
//    	double[] myVal = new double[] {2,3,1,4};
//    	double[] myNew = new double[4];
//    	
//     	runSequential(1, myNew, myVal, 2);
//     	for(double i : myNew) {
